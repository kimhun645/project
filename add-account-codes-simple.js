// Simple script to add account codes to Firestore
// This script should be run in the browser console on the Firebase project
...
// Function to add account codes to Firestore
async function addAccountCodesToFirestore() {
  console.log('🚀 เริ่มเพิ่มข้อมูล Account Codes...');
  
  try {
    // ใช้ Firebase Web SDK ที่มีอยู่แล้วในหน้าเว็บ
    const { getFirestore, collection, addDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
    
    // ใช้ Firebase app ที่มีอยู่แล้ว
...
window.addAccountCodesToFirestore = addAccountCodesToFirestore;

console.log('📋 Account Codes data loaded. Run addAccountCodesToFirestore() to add to Firestore.');
